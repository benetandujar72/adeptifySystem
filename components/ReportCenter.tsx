...
const ReportCenter: React.FC = () => {
  ...
  const mockPreviewData = [
    { metric: 'Mitjana Global', value: '7.8' },
    { metric: 'Tasques Lliurades', value: '92%' },
    { metric: 'Assistència mitjana', value: '94.5%' },
    { metric: 'Incidències resoltes', value: '14' },
  ];
  ...
  const generateReport = (format: 'PDF' | 'CSV') => {
    ...
    setTimeout(() => {
      setGenerating(null);
      alert(`Informe mensual en format ${format} generat i llest per a la descàrrega.`);
    }, 2000);
  };
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
      ...
      <div className="mb-6 p-4 bg-slate-50 rounded-xl border border-slate-100">
        <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Previsualització de dades</h4>
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between p-4 bg-indigo-50 rounded-xl border border-indigo-100 mt-4">
          <div>
            <span className="block text-sm font-bold text-indigo-900">Programació Automàtica</span>
            <span className="text-[10px] text-indigo-700">Generat cada dia 1 del mes a les 08:00h</span>
          </div>
          <button
