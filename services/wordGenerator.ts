...
export class WordProposalGenerator {
  async generate(data: any, images: { mockups: string[], workflow: string }): Promise<Buffer> {
      ...
      sections: [
        // SECCIÓ 1: PORTADA
        {
          ...
          children: [
            ...
            new Paragraph({ spacing: { before: 400 }, alignment: AlignmentType.CENTER, children: [
              new TextRun({ text: data.proyecto.titulo.toUpperCase(), bold: true, size: 28, color: COLORS.SECONDARY })
            ]}),
        ...
        // SECCIÓ 2: CONTINGUT PRINCIPAL
        {
          ...
          children: [
            ...
            new Paragraph({ children: [new TextRun(data.proyecto.resumen)] }),
            ...
            new Paragraph({ children: [new TextRun(data.diagnostico.entorno)] }),
            
            // 3. SOLUCIÓ PROPOSADA + MOCKUPS
            new Paragraph({ heading: HeadingLevel.HEADING_1, children: [new TextRun("3. SOLUCIÓ PROPOSADA")] }),
            new Paragraph({ children: [new TextRun(data.solucion.vision)] }),
            
            // Inserció de Mockups (Imatges IA)
            ...images.mockups.map((imgBase64, idx) => ([
              new Paragraph({ heading: HeadingLevel.HEADING_2, children: [new TextRun(`Visualització Solució - Mockup ${idx + 1}`)] }),
              new Paragraph({ 
            ...
            new Table({
              ...
              rows: [
                ...
                ...data.economia.conceptos.map((c: any, i: number) => new TableRow({
                  children: [
            ...
            new Paragraph({ spacing: { before: 400 }, children: [
              new TextRun({ text: `INVERSIÓ TOTAL: ${data.proyecto.inversion_total}`, bold: true, color: COLORS.PRIMARY, size: 24 })
            ]}),

            // 12. PROXIMS PASSOS I SIGNATURA
            new Paragraph({ children: [new PageBreak()] }),
