...
const AdeptifyChat: React.FC<AdeptifyChatProps> = ({ centerId = 'default_user' }) => {
  ...
  // Funció per renderitzar el text de forma visual sense mostrar el Markdown
  const renderFormattedText = (text: string) => {
    return text.split('\n').map((line, index) => {
      ...
      if (line.trim().startsWith('- ') || line.trim().startsWith('* ')) {
        ...
        // Gestionem negreta dins de bullets
      ...
      {isOpen && (
        <div className="w-80 md:w-[450px] h-[650px] bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 mb-4 flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
          ...
          <div className="bg-slate-900 p-6 text-white flex justify-between items-center shrink-0">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">Base de dades oficial activa</p>
              </div>
            </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#F8FAFC] custom-scrollbar">
            {messages.length === 0 && (
              <div className="text-center py-20 px-10">
                ...
                <p className="text-[10px] font-black uppercase text-indigo-500 tracking-[0.3em] mb-3 italic">Adeptify Knowledge Center</p>
                <p className="text-sm text-slate-500 leading-relaxed font-bold italic">Tinc tota la informació sobre els nostres sistemes de Checklists, Visió IA i LMS Offline. Com et puc ajudar?</p>
              </div>
            )}
          ...
          <div className="p-6 bg-white border-t border-slate-100 shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
            ...
            <p className="text-[8px] text-slate-400 font-bold uppercase tracking-widest mt-3 text-center opacity-60 italic">AI Powered by Adeptify Engineering</p>
          </div>
        </div>
      )}
